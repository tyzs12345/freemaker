...
//	}
	...
	public static Class getInheritClass(Class clazz){
		try {
    ...
    //拷贝对象的成员变量的值到另一个同类型的对象中
    public static Object objToObj(Object yuanShiObj, Object muBiaoObj){
    	//获取对象类型
    	Class c = yuanShiObj.getClass();
    	Class cm = muBiaoObj.getClass();
    	try {
			//获取所有成员（包括private）的值，并拷贝到新对象中
    		List<Field> fields = getProperty(c);
    		List<Field> fieldsCM = getProperty(cm);
			...
			for(Field f : fields){
				...
				fieldValue = f.get(yuanShiObj);
				map.put(s[s.length-1], fieldValue);
//				f.set(muBiaoObj, fieldValue);
			}
			for(Field f : fieldsCM){
				//不拷贝static成员和final成员
				if(Modifier.isStatic(f.getModifiers())||Modifier.isFinal(f.getModifiers())){
					continue;
				}
				f.setAccessible(true);
//				fieldValue = f.get(yuanShiObj);
				String[] s = f.toString().replace(".", " ").split(" ");
				Object obj = map.get(s[s.length - 1]);
				Object objmb = f.get(muBiaoObj);
	...
	private static List<Field> getProperty(Class c) {
		...
		if (c.getGenericSuperclass() != null) {
			List<Field> fs = getProperty(c.getSuperclass());
			for (Field field : fs) {
				fieldList.add(field);
			}
		}
